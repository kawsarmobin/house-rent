<button type="button" class="badge badge-secondary" data-toggle="modal"
  data-target="#word_edit-{{ $id }}">
  <i class="fa fa-edit"></i>
</button>
<!-- Modal -->
<div class="modal fade" id="word_edit-{{ $id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true">
                        ...
                        <option value="">Select Village</option>
                            @if ($villages)
                                @foreach ($villages as $village)
                                    <option {{ $village->id == $village_id ? 'selected' : ''  }} value="{{ $village->id }}">{{ $village->village }}</option>
                                @endforeach
                            @endif
                    </select>
                </div>
